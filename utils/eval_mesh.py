...
def eval_mesh(file_pred, file_trgt, scene_config, is_mesh, threshold=.1, bbx_name='eval_bbx', use_o3d=True, save_name="eval"):
    ...
    if "sfm_path" in scene_config.keys():
        print("cropping with sfm points...")
        sfm_filtered = filtered_sfm(scene_config["sfm_path"], sfm_to_gt=np.array(scene_config['sfm2gt']), track_length=5, reproj_error=0.8, save_path=f"{save_dir}/sfm_points.ply")
        verts_pred = point_crop(verts_pred, sfm_filtered, 0.04, scene_config[bbx_name], batch_size=8, save_path=f"{save_dir}/pred_filtered.ply")
        verts_trgt = point_crop(verts_trgt, sfm_filtered, 0.04, scene_config[bbx_name], batch_size=8, save_path=f"{save_dir}/target_filtered.ply")
    ...
    for i, _ in enumerate(threshold):
        save_path =  os.path.join(save_dir, "visualize", f"{threshold[i]}")
